...
if [ $(ls -1A /var/www/ | wc -l) -eq 0 ]; then
	...
	wp config set --raw WP_REDIS_PORT $REDIS_PORT
	wp config set --raw WP_REDIS_TIMEOUT 1
	wp config set --raw WP_REDIS_READ_TIMEOUT 1
	wp config set --raw WP_REDIS_DATABASE 0
